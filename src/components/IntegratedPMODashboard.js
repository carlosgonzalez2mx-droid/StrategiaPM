...
const IntegratedPMODashboard = ({ 
  ...
  // Estados del proyecto y datos base (MANTENIDOS para compatibilidad)
  const [staticProjects] = useState([
    {
      id: 'PRJ001',
  ...
  // Usar datos reales si están disponibles, sino usar datos estáticos
  const projects = realProjects.length > 0 ? realProjects : staticProjects;

  // Usar proyecto actual del sistema si está disponible, sino usar el primero
  const [selectedProjectId, setSelectedProjectId] = useState(
    currentProjectId || (projects.length > 0 ? projects[0].id : 'PRJ001')
  );
  ...
  const projectMetrics = useMemo(() => {
    ...
    // Calcular métricas reales si hay work packages
    let realProgress = progress;
    let realBudgetUtilization = 65; // Valor por defecto
    let realRiskLevel = selectedProject.priority === 'Alta' ? 'Alto' : 'Medio';
    
    if (workPackages.length > 0) {
      // Calcular progreso real basado en work packages
      const completedWP = workPackages.filter(wp => wp.status === 'completed').length;
      realProgress = workPackages.length > 0 ? (completedWP / workPackages.length) * 100 : progress;
      
      // Calcular utilización de presupuesto real
      const totalBudget = workPackages.reduce((sum, wp) => sum + (wp.budget || 0), 0);
      const actualCost = workPackages.reduce((sum, wp) => sum + (wp.actualCost || 0), 0);
      realBudgetUtilization = totalBudget > 0 ? (actualCost / totalBudget) * 100 : 65;
    }
    
    // Calcular nivel de riesgo real si hay riesgos
    if (risks.length > 0) {
      const highRiskCount = risks.filter(r => r.priority === 'high').length;
      const totalRisks = risks.length;
      realRiskLevel = totalRisks > 0 && (highRiskCount / totalRisks) > 0.5 ? 'Alto' : 'Medio';
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              ...
              {/* Métricas en tiempo real si hay datos del sistema */}
              {workPackages.length > 0 && (
                <div className="border-t pt-3 mt-3">
                  <div className="text-xs text-gray-500 mb-2">📈 Datos en Tiempo Real</div>
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-600">Work Packages:</span>
                    <span className="text-xs font-medium text-gray-800">
                      {projectMetrics.completedWorkPackages}/{projectMetrics.workPackagesCount}
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm font-medium text-gray-800">{projectMetrics.budgetUtilization.toFixed(1)}%</span>
              </div>
              <div className="flex justify-between">
              ...
              {/* Métricas de riesgos en tiempo real */}
              {risks.length > 0 && (
                <div className="border-t pt-3 mt-3">
                  <div className="text-xs text-gray-500 mb-2">⚠️ Riesgos Activos</div>
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-600">Total:</span>
                    <span className="text-xs font-medium text-gray-800">{projectMetrics.activeRisks}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-600">Alta Prioridad:</span>
                    <span className="text-xs font-medium text-red-600">{projectMetrics.highPriorityRisks}</span>
                  </div>
                </div>
              )}
            </div>
